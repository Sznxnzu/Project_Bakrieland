...
# Direktori untuk menyimpan hasil sementara
...
def display_mobile_results(session_id):
    ...
    <style>
        ...
        .mood-box-content {
            ...
            padding: 15px; border-radius: 10px; box-shadow: 0 0 20px #00f0ff;
            font-size: 15px; margin-top: 10px; margin-bottom: 20px;
        }
        .header-box {
            text-align: center; border: 2px solid #00f0ff; background-color: rgba(0,0,50,0.5);
            border-radius: 8px; padding: 10px; margin-bottom: 10px;
            box-shadow: 0 0 10px #00f0ff; color: #00f0ff; font-size: 20px;
        }
        img { max-width: 100%; border-radius: 8px; }
        .download-button {
            display: block; width: 100%; padding: 15px; margin-top: 20px;
            background-color: #00c0cc; color: #000; font-weight: bold; text-align: center;
            border: none; border-radius: 8px; cursor: pointer; text-decoration: none; font-size: 18px;
        }
    </style>
    ...
    try:
        ...
        result_path = os.path.join("temp_results", f"{session_id}.json")
        ...
        <div class="mood-box-content">
          <h2 style="font-size: 28px;">Mood Analytic</h2>
        ...
        st.error(f"Terjadi kesalahan saat memuat hasil: {e}")
                ...
                with st.spinner("Menganalisis suasana hati Anda..."):
                    try:
                        ...
                        if len(filenames) >= 4:
                            ...
                            with open(os.path.join("temp_results", f"{session_id}.json"), 'w') as f:
...
if "session_id" in query_params:
    display_mobile_results(query_params["session_id"])
else:
    run_main_app()